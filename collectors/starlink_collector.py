...
def make_stub():
    ...
    return stub, Request, GetStatusRequest


def read_starlink(stub, Request, GetStatusRequest) -> dict | None:
    try:
...
def main() -> None:
    ...
    stub, Request, GetStatusRequest = make_stub()
    discovery_done = False

    while True:
        data = read_starlink(stub, Request, GetStatusRequest)
        if data:
            ...
            log.info("↓%s Mbit/s  ↑%s Mbit/s  ping=%sms  drop=%s%%  obstr=%s%%",
